...
fetch("https://6646535251e227f23aae9ab7.mockapi.io/v1/posts").then((response)=>response.json()).then((response)=>{
    response.forEach((item)=>{
        (0, _functionsJs.createCard)(item.id, item.image, item.avatar, item.hashtag, columns);
    });
    return response;
}).then((data)=>{
    return cardData = {
...
columns.addEventListener("click", (event)=>{
    ...
    if (event.target.classList.contains("overlay__save")) (0, _functionsJs.showModal)(currentCard);
    if (event.target.classList.contains("modal__close")) {
        const modal = columns.querySelector(".modal");
        modal.remove();
    }
    if (event.target.classList.contains("overlay__report")) (0, _functionsJs.showReport)(report);
    if (event.target.classList.contains("boards-container__board")) (0, _functionsJs.saveData)(event);
});
searchField.addEventListener("input", (event)=>{
    (0, _functionsJs.search)(event, columns, cardData);
});
...
function aspectRatio(id, card) {
    ...
    if (id % 2 === 0) img.classList.add("two");
...
},{}]},["farZc","8lqZg"], "8lqZg", "parcelRequire1025")
